...
class Bank:
    def __init__(self, name, unit, value = 0):
        self.name = name # what type? is a string
        self.unit = unit # what type? is a class that we just defined
        self.current_account = unit(value) # instance of a currency class  ex: 10 Pounds (or whatver the unit is)
        self.value = value
    ...
    def deposit(self, inst_currency): #instance of a currency
        if isinstance(inst_currency, self.unit): #compare dollar to dollar. Does this instance come from the same class as "Dollar" "Pound" "Yuan"?
            self.current_account.value += inst_currency.value
